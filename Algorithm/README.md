...
O(N^2)의 시간이 소요되는 정렬 알고리즘은 대표적으로 버블 정렬, 선택 정렬, 삽입 정렬이 있습니다. O(N^2)은 데이터가 많아지면 매우 느려지는 알고리즘입니다.

#### 1. 버블 정렬(Bubble Sort)
버블 정렬은 인접한 두 개의 원소를 서로 비교한 후, 조건이 맞지 않으면 교환하는 방식으로 동작합니다. 구현이 간단하고 코드가 직관적인 장점이 있습니다.
...
선택 정렬은 자리를 지정해놓고, 그 자리에 들어갈 원소를 선택하는 방식으로 동작합니다. 선택 정렬 역시 버블 정렬과 같이 단순하다는 장점이 있습니다.
...
삽입 정렬은 2번째 원소부터 삽입될 데이터로 따로 저장해두고, 그 앞의 원소들과 비교하면서 자신이 삽입될 자리를 찾습니다. 앞의 원소들과 비교하면서 조건이 맞지 않는 경우 바로 뒤의 원소와 교환하면서 자리를 만들어냅니다.

삽입 정렬은 대부분 정렬이 되어 있는 경우에는 코드 상 두 번째 for문이 거의 동작하지 않으므로 O(N)의 속도에 근접합니다. 그래서 정렬 라이브러리에서 자주 사용됩니다.
...
#### 1. 병합 정렬(Merge Sort)
병합 정렬은 **분할 정복(Divide and conquer)** 방법을 사용한 대표적인 알고리즘입니다. 분할 정복은 말 그대로 문제를 분할한 후 정복(해결)한다는 방법입니다. 복잡한 문제를 분할하여 단순화한 후 해결하는 것이 목적입니다.

병합 정렬 역시 데이터가 1개가 될 때까지 분할한 후, 정복하는 과정에서 데이터를 합치면서 비교하여 정렬합니다.
...
#### 2. 퀵 정렬(Quick Sort)
퀵 정렬은 이름대로 정렬 알고리즘 중에서 평균적으로 가장 빠른 알고리즘입니다. 그리고 병합 정렬에 비교해서 임시적인 배열도 필요없습니다. 하지만 최악의 시간 복잡도는 O(N^2)이라는 단점을 가지고 있습니다.
...
퀵 정렬의 worst case는 pivot이 partition 결과로 배열의 가장 왼쪽이나 가장 오른쪽에 지속해서 위치하는 경우입니다. 이는 pivot 기준으로 분할할 때 n개에서 (n-1), (n-2), (n-3), ... 으로 나뉘기 때문에 시간 복좁다는 O(N^2)과 같습니다.
...
이를 이용해서 최대 힙은 오름차순으로, 최소 힙은 내림차순으로 사용할 수 있습니다. 그리고 힙을 삭입, 삭제하는데 O(logN)의 시간복잡도가 소요되므로, N개의 데이터를 힙으로 만든 뒤, 모두 꺼내는데 총 O(NlogN)의 시간이 소요됩니다.

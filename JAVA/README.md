...
## Q. Java8에서 추가된 기능
자바8을 사용하면서 자주 사용하는 기능은 람다, 스트림API, Optional 이었습니다. 그 외에는 default 키워드, completable future, date type이 있는 것으로 알고 있습니다.
...
### 꼬리질문 1-1. 익명 클래스나 람다를 사용하는 이유가 무엇인가요?
익명 클래스나 람다를 사용하는 이유는 재사용할 필요없는 일회성 로직을 처리할 때 유용합니다. 이러한 일회성을 위해 불필요한 클래스 파일이 늘어나는 문제점을 해결할 수 있다고 생각합니다.
...
### 꼬리질문 2. Stream API
스트림은 java8에서 컬렉션 데이터를 선언형으로 처리하는 API입니다. 선언형은 SQL 질의어와 같이 내부 동작이 어떻게 동작하는지 신경쓰지않고 무엇을 하는지를 직접적으로 표현합니다.

### 꼬리질문 2-1. 스트림의 장점
스트림 API를 사용함으로써 세 가지 장점을 얻을 수 있습니다.
1. 선언형이므로 위에서 말했듯이, 간결하고 가독성을 높일 수 있습니다.
2. map, collet와 같은 여러 api를 제공하여 이를 조립하며 사용할 수 있습니다.
3. 병렬을 처리해주는 기능을 제공하여 좀 더 안전하고 성능을 높일 수 있습니다.

### 꼬리질문 2-2. 스트림의 특징
스트림 API의 특징은 두 가지가 있습니다.
1. 파이프라이닝이 가능합니다. 스트림 연산은 스트림을 반환할 수 있기 때문에 이를 연결해서 파이프라인을 구성할 수 있습니다. 따라서 레이지하게 만들어 필요한 순간에만 데이터가 올라오도록 최적화를 할 수 있습니다.
2. 기존의 for문이 외부 반복을 하는 것과는 달리 내부 반복을 수행합니다.

### 꼬리질문 2-3. 스트림과 컬렉션의 차이
...
### 꼬리질문 3. Optional
Optional은 널의 위험성을 해결하기 위해 객체를 한 번더 캡슐화하는 클래스입니다. Optional은 객체가 비어있다면 null이 아닌 Optional.empty()라는 메서드를 반환합니다. Optional을 사용함으로써 해당 객체가 없을 수 있다는 것을 알려주는 역할을 하기도 합니다.

### 꼬리질문 3-1. NULL때문에 발생하는 문제점은?
자바에서 NULL의 가장 큰 문제는 NullPointException이라고 생각합니다. 이는 NULL인 객체를 참조하려고 할 때 발생합니다. NULL이라는 것은 값이 없음을 의미할 수도 있고, 개발자의 실수일 수도 있습니다. 따라서 어떤 의미인지 알기 힘듭니다.

...
Web Server는 여러 대의 WAS 서버를 가지고 로드 밸런싱 역할도 할 수 있습니다. 대표적으로 Nginx가 이러한 역할로 많이 사용되고 있습니다.
...
Web Server가 필요한 실제 사례로는 다음과 같다고 합니다.
...
**인증**은 사용자의 신원을 증명하는 것입니다. 대표적으로 로그인은 사용자가 누구인지 인증하는 과정입니다. **인가**는 특정 리소스에 접근할 수 있는 권한을 부여하는 것입니다. 로그인을 통해 인증된 사용자일지라도 특정 리소스에 접근할 때는 권한이 있는지 검사하는 인가 과정이 필요합니다.

### 인증 방식
인증은 일반적으로 회원가입을 완료한 회원에 대해 현재 회원이 맞는지를 검사하기 위함입니다. 최초로 회원이 맞는지 검사하는 로그인이 있고, HTTP는 기본적으로 상태를 저장하지 않는 프로토콜이므로 로그인 이후에도 현재 회원이 맞는지 인증과정을 계속 확인해야 합니다.
